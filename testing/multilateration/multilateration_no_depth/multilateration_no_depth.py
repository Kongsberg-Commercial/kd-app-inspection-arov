...
def multilaterate(positions, distances):
...
# ------------------------------------------------------------
# Main Receiver Loop: Query distances and perform 2D multilateration
# ------------------------------------------------------------
async def main():
    ...
    origin_lat, origin_lon = transponder_latlon[0]  # Origo

    # Convert transponder positions from lat/lon to meters.
    # Although a fixed z is set (e.g., -1), we will use only x,y for 2D multilateration.
    transponder_positions = []
    for i, (lat, lon) in enumerate(transponder_latlon):
        z = -1
        x, y = latlon_to_meters(lat, lon, origin_lat, origin_lon)
        # Cheesy hack to solve geometry issues when transponders are collinear.
        # We must put one transponder deeper than the others!
        if i == 1:
            z = -6
        transponder_positions.append([x, y, z])
    transponder_positions = np.array(transponder_positions)
    
    try:
        print("Receiver started. Polling transponders and performing 2D multilateration...\n")
        ...
        while True:
            ...
            # Proceed only if all distances were received.
            if None not in distances:
                distances = np.array(distances)
                est = multilaterate(transponder_positions, distances)
                est_lat, est_lon = meters_to_latlon(est[0], est[1], origin_lat, origin_lon)
                print("\n[2D Multilateration Result]")
                print("Estimated position (meters, x,y):", est)
                print("Estimated position (lat, lon):    ", est_lat, est_lon)
                print("-" * 95)
            else:
