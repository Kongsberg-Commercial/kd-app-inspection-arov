...
Receives platform data via MQTT and controls a vehicle using MAVLink.
Supports simulation mode to test logic without sending real commands.
"""
...
# Only connect MAVLink if not in simulation mode
...
def control(x=0, y=0, z=500, r=0):
    if SIMULATION_MODE:
        print(f"[SIMULATION] control(x={x}, y={y}, z={z}, r={r})")
        return
...
def arm_vehicle():
    if SIMULATION_MODE:
        print("[SIMULATION] Skipping arming")
        return
...
def disarm_vehicle():
    if SIMULATION_MODE:
        print("[SIMULATION] Skipping disarming")
        return
    master.mav.command_long_send(
        master.target_system,
        master.target_component,
