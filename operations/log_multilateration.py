...
# Write initial message to log file, indicating that we are starting a new "session"
with open("position_log.txt", "a") as logfile:
    logfile.write(f"\nInitializing new session at {time.time()}\n")

with open("globalpos_log.txt", "a") as logfile:
    logfile.write(f"\nInitializing new session at {time.time()}\n")

# Helper function to write data to a log file 
def log_data(timestamp, x, y, z, lat, lon):
    with open("position_log.txt", "a") as logfile:
        logfile.write(f"LOCAL POSITION, {timestamp}, {x:.3f}, {y:.3f}, {z:.3f}, {lat:.7f}, {lon:.7f}\n")
...
# MAVLink listener thread: listen for different mavlink messages and handle the data accordingly.
def mavlink_listener():
    global latest_pressure, latest_temp, mav
    last_global_log_time = 0  # Variable to track last logging time for global pos
    GLOBAL_LOG_INTERVAL = 1.0 # How often we want to log (2 seconds, same as local)
    while True:
        ...
        if msg:
            ...
            if msg_type == "GLOBAL_POSITION_INT":
                ...
                # This message has to be throttled because it logs too fast!
                if current_time - last_global_log_time >= GLOBAL_LOG_INTERVAL:
                    last_global_log_time = current_time
                    timestamp = current_time
                    lat = msg.lat / 1e7
                    lon = msg.lon / 1e7
                    with open("globalpos_log.txt", "a") as logfile:
                        logfile.write(f"GLOBAL_POSITION_INT, {timestamp}, {lat:.7f}, {lon:.7f}\n")
...
async def main():
    ...
    transponder_latlon = [
        (59.428465173, 10.465137681),  # t1  (currently -5 meters) #
        (59.429155409, 10.464601094),  # far (currently origo)
        ...
        #(59.428407132, 10.465439580), # old t4
    ]
    origin_lat, origin_lon = transponder_latlon[1]  # Use far transponder as the origin for better geometry

    # Convert transponder positions from lat/lon to meters and assign depths.
    ...
    while True:
        ...
        if all(d is not None and d != 0.0 for d in distances) and new_valid_count >= min_new_required:
            ...
            log_data(timestamp, est[0], est[1], fixed_z, est_lat, est_lon)
