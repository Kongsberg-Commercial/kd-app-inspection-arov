...
topic_out     = "/hipos_output_cnode_position"
...
def make_gpgga(lat, lon):
    """Ultra-minimal GGA, same as fake_gps.py"""
...
def make_gphdt(heading):
    """Ultra-minimal HDT, same as fake_heading.py"""
...
def udp_loop():
    while True:
        ...
        print(f"[bridge → MQTT] topic={topic_out} {payload_str}")
        client.publish(topic_out, payload_str)
...
if __name__ == '__main__':
    ...
    logging.info("HiPOS↔MQTT bridge running")
    try:
        while True:
            time.sleep(1)
